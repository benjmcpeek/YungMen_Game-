...
pygame.display.set_caption('Yungmen: Thrust to Perfection')
...
#music loading 
pygame.mixer.music.load('sounds/song/parking_lot.mp3')
pygame.mixer.music.set_volume(0.3)
pygame.mixer.music.play(-1, 0.0, 2000)

box_int = pygame.mixer.Sound('sounds/effects/box interaction.wav')
box_int.set_volume(0.3)
gren_ex = pygame.mixer.Sound('sounds/effects/grenade explode.wav')
gren_ex.set_volume(0.1)
gren_throw = pygame.mixer.Sound('sounds/effects/grenade throw.wav')
gren_throw .set_volume(0.3)
injured = pygame.mixer.Sound('sounds/effects/injured.wav')
injured.set_volume(0.3)
jump = pygame.mixer.Sound('sounds/effects/jump.wav')
jump.set_volume(0.3)
notes = pygame.mixer.Sound('sounds/effects/notes.wav')
notes.set_volume(0.3)
running = pygame.mixer.Sound('sounds/effects/running.wav')
running.set_volume(0.3)
tv_death = pygame.mixer.Sound('sounds/effects/tv death.wav')
tv_death.set_volume(0.3)


#load images
start_img = pygame.image.load('img/button/0.png').convert_alpha()
start_img = pygame.transform.rotozoom(start_img, 0, 2.5)
...
class Minion(pygame.sprite.Sprite):
    def __init__(self, scale, y_pos, health):
        pygame.sprite.Sprite.__init__(self)
        ...
        self.update_time = pygame.time.get_ticks()
        self.max_health = self.health
        self.animation_types = ['minion', 'minion_death']
    ...
    def update(self, y_pos):
        ...
        if self.alive == True:
            ...
            self.animation_types[0]
        self.check_alive()
        if self.alive == False:
            self.rect.y += 8
            self.animation_state()
            self.update_action(1)
            self.animation_types[1]
        self.y_pos += 1
        if self.shoot_cooldown > 0:
            self.shoot_cooldown -= 1
        self.rect.x += screen_scroll
...
start_button = button.Button(SCREEN_WIDTH // 2 - 100, SCREEN_HEIGHT // 2 + 160, start_img, 1)
...
#minion = Minion(0, any, health = 1000, rect)
#minion_1 = Minion(0, any, health = 1000, any)
...
pygame.time.set_timer(ob_timer, 10000)
...
while run:
    ...
    if start_game == False:
        screen.fill(BG)
        player_img = pygame.image.load('img/player/Shoot_Run//6.png').convert_alpha()
        player_img = pygame.transform.rotozoom(player_img, 0, 2.7)
        screen.blit(player_img, (30 + (x * 15), 150))
    ...
    for event in pygame.event.get():
        ...
        if event.type == ob_timer:
            minion_group.add(Minion(0, health=100, y_pos=50))
